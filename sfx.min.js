...
c[a][d].play()}};f.loop=function(a,b){l(a,"loop",!0);l(a,"currentTime",0);f.play(a,b)};f.pause=function(a,b){if(h&&c[a])for(var d=0;d<c[a].length;d++)c[a][d].pause(),c[a][d].loop=!1,b&&(c[a][d].currentTime=0)};f.toggle=function(a,b){if(h&&c[a]){for(var d=0,e=0;e<c[a].length;e++)c[a][e].paused||d++;d?f.pause(a):f.play(a,b)}};f.stop=function(a){f.pause(a,1)};f.volume=function(a,b){if(h&&(b="string"===typeof a?b:a,a="string"===typeof a?a:!1,!isNaN(parseFloat(b))&&isFinite(b)&&!(0>b||100<b)))if(a)l(a,
"volume",j/100*b/100);else{j=b;for(var d in c)l(d,"volume",j/100*(c[d][0].volume/1))}};f.mute=function(a,b){b="boolean"===typeof b?b:"boolean"===typeof a?a:!0;if(a="string"===typeof a?a:!1)b&&f.pause(a),c[a].muted=b;else{if(b)for(var d in c)f.pause(d);p=b}};(function(){var a=document.createElement("audio");try{if(h=!!a.canPlayType)h=new Boolean(h),h.ogg=a.canPlayType('audio/ogg; codecs="vorbis"').replace(/^no$/,""),h.mp3=a.canPlayType("audio/mpeg;").replace(/^no$/,"")}catch(b){}k&&(h=!!h[k.toLowerCase()]);
m=k?k.toLowerCase():h.ogg?"ogg":"mp3";o=n?n.replace(/\/+$/,"").replace(/\{\{type\}\}/,m):""})()};
